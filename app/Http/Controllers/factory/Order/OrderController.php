...
class OrderController extends BaseController
{
    ...
    public function getListView(object $request): View
    {
        ...
        $this->orderRepo->updateWhere(params: ['seller_id' => $factoryId, 'checked' => 0], data: ['checked' => 1]);
        ...
        $sellers = $this->factoryRepo->getByStatusExcept(status: 'pending', relations: ['shop']);
    ...
    public function generateInvoice(string|int $id): void
    {
        ...
        $factoryId = auth('seller')->id();
        $factory = $this->factoryRepo->getFirstWhere(params: ['id' => $factoryId])['gst'];

        $params = ['id' => $id, 'seller_id' => $factoryId, 'seller_is' => 'seller'];
        ...
        $mpdf_view = PdfView::make(Order::GENERATE_INVOICE[VIEW],
            compact('order', 'factory', 'companyPhone', 'companyEmail', 'companyName', 'companyWebLogo')
        );
