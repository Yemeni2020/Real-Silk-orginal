...
namespace App\Http\Controllers\factory\POS;
...
class POSController extends BaseController
{
    ...
    public function getPOSView(object $request):View
    {
        ...
        $factory = $this->factoryRepo->getFirstWhere(params: ['id' => $factoryId]);
        $getPOSStatus = getWebConfig('seller_pos');
        if ($factory['pos_status'] == 0 || $getPOSStatus == 0) {
            Toastr::warning(translate('access_denied!!'));
        }

        $shop = $this->shopRepo->getFirstWhere(params: ['id' => $factoryId]);
