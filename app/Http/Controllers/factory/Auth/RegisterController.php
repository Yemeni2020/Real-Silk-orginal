...
class RegisterController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
                ...
                'email'             => 'required|unique:factories',
        ...
        if($request['from_submit'] != 'admin') {
            ...
            if (isset($recaptcha) && $recaptcha['status'] == 1) {
                ...
                if (strtolower($request->default_recaptcha_id_seller_regi) != strtolower(Session(SessionKey::factory_RECAPTCHA_KEY))) {
        ...
        DB::transaction(function ($r) use ($request) {
            ...
            $seller->image = ImageManager::upload('factory/', 'webp', $request->file('image'));
            $seller->password = bcrypt($request->password);
            $seller->status =  $request->status == 'approved'?'approved': "pending";
            $seller->save();

            $shop = new Shop();
            $shop->factory_id = $seller->id;
            ...
            DB::table('factory_wallets')->insert([
                'factory_id' => $seller['id'],
                'withdrawn' => 0,
                'commission_given' => 0,
                'total_earning' => 0,
                'pending_withdraw' => 0,
