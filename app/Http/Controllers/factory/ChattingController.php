...
use App\Http\Requests\factory\ChattingRequest;
...
class ChattingController extends BaseController
{
    ...
    public function getListView(string|array $type):View
    {
        ...
        if ($type == 'delivery-man') {
            $lastChat = $this->chattingRepo->getFirstWhereNotNull(
                params: ['seller_id' =>$factoryId],
                filters: ['delivery_man_id', 'seller_id'],
                orderBy: ['created_at' => 'DESC']
            );
            if (isset($lastChat)) {
                $this->chattingRepo->updateAllWhere(
                    params: ['seller_id' => $factoryId, 'delivery_man_id' => $lastChat['delivery_man_id']],
            ...
            if (isset($lastChat)) {
                $this->chattingRepo->updateAllWhere(
                    params: ['seller_id' => $factoryId, 'user_id' => $lastChat['user_id']],
                    data: ['seen_by_seller' => 1]);
    ...
    public function getMessageByUser(Request $request):JsonResponse
    {
        ...
        if ($request->has(key: 'delivery_man_id')) {
            $this->chattingRepo->updateAllWhere(
                params: ['seller_id' => $factoryId, 'delivery_man_id' => $request['delivery_man_id']],
                data: ['seen_by_seller' => 1]);
            $chattings = $this->getChatList(
                tableName: 'delivery_men',
                orderBy : 'asc',
            ...
            $this->chattingRepo->updateAllWhere(
                params: ['seller_id' => $factoryId, 'user_id' => $request['user_id']],
                data: ['seen_by_seller' => 1]
            );
            $chattings = $this->getChatList(
                tableName: 'users',
     ...
     * @return JsonResponse
     */
    public function addfactoryMessage(ChattingRequest $request):JsonResponse
    {
        ...
        if ($request->has(key: 'delivery_man_id')) {
            ...
            ChattingEvent::dispatch('message_from_seller', 'delivery_man', $deliveryMan, $factory);
        } elseif ($request->has(key: 'user_id')) {
            $this->chattingRepo->add(
            ...
            ChattingEvent::dispatch('message_from_seller', 'customer', $customer, $factory);
        }
        $imageArray = [];
    ...
    protected function getChatList(string $tableName, string $orderBy, string|int $id = null) :Collection
    {
        ...
        $filters = isset($id) ? ['chattings.seller_id' => $factoryId, $columnName => $id] : ['chattings.seller_id' => $factoryId];
