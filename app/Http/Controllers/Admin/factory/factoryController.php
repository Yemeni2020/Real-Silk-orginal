...
use App\Enums\ExportFileNames\Admin\factory as factoryExport;
use App\Enums\ViewPaths\Admin\factorys;
...
class factoryController extends BaseController
{
    ...
    public function getListView(Request $request): View
    {
        ...
        $sellers = $this->factoryRepo->getListWhere(
            orderBy:['id'=>'desc'],
            searchValue: $request['searchValue'],
            relations: ['orders', 'product'],
            dataLimit:getWebConfig(name: WebConfigKey::PAGINATION_LIMIT)
        );
        return view(factorys::LIST[VIEW], compact('sellers', 'current_date'));
    }

    public function getAddView(Request $request): View
    {
        return view(factorys::ADD[VIEW]);
    }

    public function updateStatus(Request $request): RedirectResponse
    {
        $this->factoryRepo->update(id: $request['id'], data: ['status' => $request['status']]);
        if ($request['status'] == "approved") {
            Toastr::success(translate('factory_has_been_approved_successfully'));
        } else if ($request['status'] == "rejected") {
            Toastr::info(translate('factory_has_been_rejected_successfully'));
        } else if ($request['status'] == "suspended") {
            $this->factoryRepo->update(id: $request['id'], data: ['auth_token' => Str::random(80)]);
            Toastr::info(translate('factory_has_been_suspended_successfully'));
        }
        return back();
    }
    ...
    public function exportList(Request $request): BinaryFileResponse
    {
        ...
        return Excel::download(new SellerListExport($data),factoryExport::EXPORT_XLSX);
    }
    ...
    public function getOrderListView(Request $request, $seller_id): View
    {
        ...
        $seller = $this->factoryRepo->getFirstWhere(params: ['id'=>$seller_id]);
        return view(factorys::ORDER_LIST[VIEW], compact('orders', 'seller'));
    }
    ...
    public function getProductListView(Request $request, $seller_id): View
    {
        ...
        $seller = $this->factoryRepo->getFirstWhere(params: ['id'=>$seller_id]);
        return view(factorys::PRODUCT_LIST[VIEW], compact('products', 'seller'));
    }
    ...
    public function updateSalesCommission(Request $request, $id): RedirectResponse
    {
        ...
        $this->factoryRepo->update(id: $id, data: ['sales_commission_percentage' => $request['commission_status'] == 1 ? $request['commission'] : null]);
    ...
    public function getOrderDetailsView($order_id, $seller_id): View
    {
        ...
        return view(factorys::ORDER_DETAILS[VIEW], compact('order', 'seller_id','delivery_men', 'linked_orders','physical_product',
    ...
    {
        $seller = $this->factoryRepo->getFirstWhere(
        ...
        if(!isset($seller)){
            Toastr::error(translate('factory_not_found_It_may_be_deleted'));
            return back();
        }
        ...
        return view(factorys::VIEW[VIEW], [
            'seller' => $seller,
            'current_date' => date('Y-m-d'),
        ]);
    }
    ...
    public function getOrderListTabView(Request $request, $seller): View
    {
        ...
        return view(factorys::VIEW_ORDER[VIEW], compact('seller', 'orders','pendingOrder','deliveredOrder'));
    }
    ...
    public function getProductListTabView(Request $request, $seller): View
    {
        ...
        return view(factorys::VIEW_PRODUCT[VIEW], compact('seller', 'products'));
    }

    public function getSettingListTabView(Request $request, $seller, $id): View
    {
        return view(factorys::VIEW_SETTING[VIEW], compact('seller'));
    }
    public function updateSetting(Request $request ,$id):RedirectResponse
    {
        ...
        if ($request->has('gst')) {
            if ($request['gst_status'] == 1 && $request['gst'] == null) {
                ...
                $this->factoryRepo->update(id: $id, data: ['gst' => $request['gst_status'] == 1 ? $request['gst'] : null]);
                Toastr::success(translate('GST_number_for_this_seller_has_been_updated.'));
            }
        }
        if ($request->has('seller_pos_update')) {
            $this->factoryRepo->update(id: $id, data: ['pos_status' => $request->get('seller_pos', 0)]);
            Toastr::success(translate('factory_pos_permission_updated'));
        }
    ...
    public function getTransactionListTabView(Request $request, $seller): View
    {
        ...
        return view(factorys::VIEW_TRANSACTION[VIEW], compact('seller', 'transactions'));
    }
    ...
    public function getReviewListTabView(Request $request, $seller): View
    {
        ...
        return view(factorys::VIEW_REVIEW[VIEW], [
            'seller' => $seller,
            'reviews' => $reviews,
        ]);
    }
    ...
    public function getWithdrawView($withdraw_id, $seller_id): View|RedirectResponse
    {
        ...
        if ($withdrawRequest) {
            ...
            return view(factorys::WITHDRAW_VIEW[VIEW], compact('withdrawRequest', 'withdrawalMethod','direction'));
        }
    ...
    public function getWithdrawListView(Request $request): View
    {
        ...
        return view(factorys::WITHDRAW_LIST[VIEW], compact('withdrawRequests'));
    }
    ...
    public function withdrawStatus(Request $request, $id): RedirectResponse
    {
        ...
        if ($request['approved'] == 1) {
            $this->factoryWalletRepo->getFirstWhere(params: ['seller_id'=>$withdraw['seller_id']])->increment('withdrawn', $withdraw['amount']);
            $this->factoryWalletRepo->getFirstWhere(params: ['seller_id'=>$withdraw['seller_id']])->decrement('pending_withdraw', $withdraw['amount']);

            $this->withdrawRequestRepo->update(id: $id, data: $withdrawData);
            Toastr::success(translate('factory_Payment_has_been_approved_successfully'));
            return redirect()->route('admin.sellers.withdraw_list');
        }

        $this->factoryWalletRepo->getFirstWhere(params: ['seller_id'=>$withdraw['seller_id']])->increment('total_earning', $withdraw['amount']);
        $this->factoryWalletRepo->getFirstWhere(params: ['seller_id'=>$withdraw['seller_id']])->decrement('pending_withdraw', $withdraw['amount']);
        $this->withdrawRequestRepo->update(id: $id, data: $withdrawData);

        Toastr::info(translate('factory_Payment_request_has_been_Denied_successfully'));
        return redirect()->route('admin.sellers.withdraw_list');
