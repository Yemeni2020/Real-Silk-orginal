...
class ChattingController extends BaseController
{
     ...
     * @return View
     */
    public function getListView(string|array $type): View|null
    {
        ...
        elseif($type == 'factory'){
            ...
            if (count($allChattingUsers) > 0) {
                ...
                $this->chattingRepo->updateAllWhere(
                    params: ['office_id' => $vendorId, 'seller_id' => $lastChatUser['id']],
                    data: ['seen_by_seller' => 1]
                );

                $chattingMessages = $this->chattingRepo->getListWhereNotNull(
                    orderBy: ['created_at' => 'DESC'],
                    filters: ['office_id' => $vendorId, 'seller_id' => $lastChatUser->id],
                    whereNotNull: ['seller_id', 'office_id'],
                    relations: ['seller'],
                    dataLimit: 'all'
                );
        ...
        elseif($type == 'admin'){
            ...
            $vendor =$this->vendorRepo->getFirstWhere(params:["id"=>$vendorId]);
            $admins=$this->adminRepo->getList();
            $chattingMessages=[];
            $lastChatUser=$admins->first();
            if (count($allChattingUsers) > 0) {
                $lastChatUser = $allChattingUsers[0]->admin;
                $this->chattingRepo->updateAllWhere(
                    params: ['office_id' => $vendorId, 'admin_id' => $lastChatUser['id']],
                    data: ['seen_by_seller' => 1]
                );

                $chattingMessages = $this->chattingRepo->getListWhereNotNull(
                    orderBy: ['created_at' => 'DESC'],
                    filters: ['office_id' => $vendorId, 'admin_id' => $lastChatUser->id],
                    whereNotNull: ['admin_id', 'office_id'],
                    relations: ['admin'],
                    dataLimit: 'all'
                );
                // dump($allChattingUsers);
        ...
        $userType=$type;
        return view(Chatting::INDEX[VIEW], compact('shop','userType'));
    }
    ...
    public function getMessageByUser(Request $request): JsonResponse
    {
        ...
        if ($request->has(key: 'delivery_man_id')) {
            ...
            $this->chattingRepo->updateAllWhere(
                params: ['office_id' => $vendorId, 'delivery_man_id' => $request['delivery_man_id']],
                data: ['seen_by_seller' => 1]);

            $chattingMessages = $this->chattingRepo->getListWhereNotNull(
                orderBy: ['created_at' => 'DESC'],
                filters: ['office_id' => $vendorId, 'delivery_man_id' => $request['delivery_man_id']],
                whereNotNull: ['delivery_man_id', 'office_id'],
                dataLimit: 'all'
            );
            ...
            $getUser = $this->customerRepo->getFirstWhere(params: ['id' => $request['seller_id']]);
            $this->chattingRepo->updateAllWhere(
                params: ['office_id' => $vendorId, 'seller_id' => $request['seller_id']],
                data: ['seen_by_seller' => 1]
            );
            $chattingMessages = $this->chattingRepo->getListWhereNotNull(
                orderBy: ['created_at' => 'DESC'],
                filters: ['office_id' => $vendorId, 'seller_id' => $request['seller_id']],
                whereNotNull: ['seller_id', 'office_id'],
                dataLimit: 'all'
            );
            $data = self::getRenderMessagesView(user: $getUser, message: $chattingMessages, type: 'customer');
    ...
    public function addVendorMessage(ChattingRequest $request): JsonResponse
    {
        ...
        // return response()->json($request->has(key: 'delivery_man_id'));
        ...
        elseif ($request->has(key: 'seller_id')) {
            $this->chattingRepo->add(
                data: $this->chattingService->getFactoryChattingData(
                    request: $request,
                    shopId: $shop['id'],
                    vendorId: $vendor['id'])
            );
            $factory = $this->vendorRepo->getFirstWhere(params: ['id' => $request['seller_id']]);
            event(new ChattingEvent(key: 'message_from_office', type: 'seller', userData: $factory, messageForm: $vendor));

            $chattingMessages = $this->chattingRepo->getListWhereNotNull(
                orderBy: ['created_at' => 'DESC'],
                filters: ['office_id' => $vendor['id'], 'seller_id' => $request['seller_id']],
                whereNotNull: ['seller_id', 'office_id'],
                dataLimit: 'all'
            );
            $data = self::getRenderMessagesView(user: $factory, message: $chattingMessages, type: 'seller');
            ...
            $factory = $this->adminRepo->getFirstWhere(params: ['id' => $request['admin_id']]);
            event(new ChattingEvent(key: 'message_from_office', type: 'admin', userData: $factory, messageForm: $vendor));

            $chattingMessages = $this->chattingRepo->getListWhereNotNull(
                orderBy: ['created_at' => 'DESC'],
                filters: ['office_id' => $vendor['id'], 'admin_id' => $request['admin_id']],
                whereNotNull: ['admin_id', 'office_id'],
                dataLimit: 'all'
            );
            $data = self::getRenderMessagesView(user: $factory, message: $chattingMessages, type: 'admin');
        }
