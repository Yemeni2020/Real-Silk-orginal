...
class PostService
{
    ...
    public function getProcessedImages(object $request): array
    {
        ...
        if ($request->file('images')) {
            foreach ($request->file('images') as $image) {
                $images = $this->upload(dir: 'post/', format: 'webp', image: $image);
                $imageNames[] = [
                    'image_name' => $images,
                    'storage' => $storage,
                ];
    ...
    public function getProcessedUpdateImages(object $request, object $product): array
    {
        ...
        if ($request->file('images')) {
            foreach ($request->file('images') as $image) {
                $imageName = $this->upload(dir: 'post/', format: 'webp', image: $image);
                $productImages[] = [
                    'image_name' => $imageName,
                    'storage' => $storage,
                ];
    ...
    private function vendor_auto_product():int{
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        return [
            ...
            'thumbnail' => $request->has('image') ? $this->upload(dir: 'post/thumbnail/', format: 'webp', image: $request['image']) : $request->existing_thumbnail,
            'thumbnail_storage_type' => $request->has('image') ? $storage : null,
            'meta_title' => $request['meta_title']??'',
            'meta_keywords' => $request['meta_keywords']??'',
            'meta_description' => $request['meta_description']??'',
            'meta_image' => $request->has('meta_image') ? $this->upload(dir: 'post/meta/', format: 'webp', image: $request['meta_image']) : $request->existing_meta_image??'',
        ];
    }
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        if ($request->file('image')) {
            $dataArray += [
                'thumbnail' => $this->update(dir: 'post/thumbnail/', oldImage: $product['thumbnail'], format: 'webp', image: $request['image'], fileType: 'image'),
...
//            'purchase_price',
    ...
    public function getProductSEOData(object $request, object|null $post = null, string $action = null): array
    {
        if ($post) {
            if ($request->file('meta_image')) {
                $metaImage = $this->update(dir: 'post/meta/', oldImage: $post['meta_image'], format: 'png', image: $request['meta_image']);
            } elseif (!$request->file('meta_image') && $request->file('image') && $action == 'add') {
                $metaImage = $this->upload(dir: 'post/meta/', format: 'webp', image: $request['image']);
            } else {
                $metaImage = $post?->seoInfo?->image ?? $post['meta_image'];
            }
        } else {
            if ($request->file('meta_image')) {
                $metaImage = $this->upload(dir: 'post/meta/', format: 'webp', image: $request['meta_image']);
            } elseif (!$request->file('meta_image') && $request->file('image') && $action == 'add') {
                $metaImage = $this->upload(dir: 'post/meta/', format: 'webp', image: $request['image']);
            }
        }
        return [
            "post_id" => $post['id'],
            "title" => $request['meta_title'] ?? ($post ? $post['meta_title'] : null),
            "description" => $request['meta_description'] ?? ($post ? $post['meta_description'] : null),
            "meta_keywords" => $request['meta_keywords'] ?? ($post ? $post['meta_keywords'] : null),
            ...
            "image" => $metaImage ?? ($post ? $post['meta_image'] : null),
            "created_at" => now(),
            "updated_at" => now(),
        ];
    }
