...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'middleware' => ['admin']], function () {
    ...
    Route::group(['prefix' => 'factorys', 'as' => 'factorys.','middleware'=>['module:user_section']], function () {
        Route::controller(factoryController::class)->group(function (){
            Route::get(factorys::LIST[URI], 'index')->name('seller-list');
            Route::get(factorys::ADD[URI], 'getAddView')->name('seller-add');
            Route::get(factorys::ORDER_LIST[URI].'/{seller_id}', 'getOrderListView')->name('order-list');
            Route::post(factorys::STATUS[URI], 'updateStatus')->name('updateStatus');
            Route::get(factorys::EXPORT[URI], 'exportList')->name('export');
            Route::get(factorys::PRODUCT_LIST[URI].'/{seller_id}', 'getProductListView')->name('product-list');

            Route::post(factorys::SALES_COMMISSION_UPDATE[URI].'/{id}', 'updateSalesCommission')->name('sales-commission-update');
            Route::get(factorys::ORDER_DETAILS[URI].'/{order_id}/{seller_id}', 'getOrderDetailsView')->name('order-details');
            Route::get(factorys::VIEW[URI].'/{id}/{tab?}', 'getView')->name('view');
            Route::post(factorys::UPDATE_SETTING[URI].'/{id}', 'updateSetting')->name('update-setting');

            Route::get(factorys::WITHDRAW_LIST[URI], 'getWithdrawListView')->name('withdraw_list');
            Route::get(factorys::WITHDRAW_LIST_EXPORT[URI], 'exportWithdrawList')->name('withdraw-list-export-excel');
            Route::get(factorys::WITHDRAW_VIEW[URI].'/{withdraw_id}/{seller_id}', 'getWithdrawView')->name('withdraw_view');
            Route::post(factorys::WITHDRAW_STATUS[URI].'/{id}', 'withdrawStatus')->name('withdraw_status');
        });
