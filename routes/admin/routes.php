...
Route::group(['prefix' => 'admin', 'as' => 'admin.', 'middleware' => ['admin']], function () {
    ...
    Route::group(['prefix' => 'post', 'as' => 'post.', 'middleware' => ['module:product_management']], function () {
        ...
        Route::controller(PostController::class)->group(function () {
            Route::get(Post::LIST[URI] . '/{type}', 'index')->name('list');
            Route::get(Post::ADD[URI], 'getAddView')->name('add');
            Route::post(Post::TRANSLATE_AI[URI], 'translate_ai')->name('translate-ai');
            Route::post(Post::ADD[URI], 'add')->name('store');
            Route::get(Post::VIEW[URI] . '/{addedBy}/{id}', 'getView')->name('view');
            Route::post(Post::SKU_COMBINATION[URI], 'getSkuCombinationView')->name('sku-combination');
            Route::post(Post::DIGITAL_VARIATION_COMBINATION[URI], 'getDigitalVariationCombinationView')->name('digital-variation-combination');
            Route::post(Post::DIGITAL_VARIATION_FILE_DELETE[URI], 'deleteDigitalVariationFile')->name('digital-variation-file-delete');
            Route::post(Post::FEATURED_STATUS[URI], 'updateFeaturedStatus')->name('featured-status');
            Route::get(Post::GET_CATEGORIES[URI], 'getCategories')->name('get-categories');
            Route::post(Post::UPDATE_STATUS[URI], 'updateStatus')->name('status-update');
            Route::get(Post::BARCODE_VIEW[URI] . '/{id}', 'getBarcodeView')->name('barcode');
            Route::get(Post::EXPORT_EXCEL[URI] . '/{type}', 'exportList')->name('export-excel');
            Route::get(Post::STOCK_LIMIT[URI] . '/{type}', 'getStockLimitListView')->name('stock-limit-list');
            Route::delete(Post::DELETE[URI] . '/{id}', 'delete')->name('delete');
            Route::get(Post::UPDATE[URI] . '/{id}', 'getUpdateView')->name('update');
            Route::post(Post::UPDATE[URI] . '/{id}', 'update');
            Route::get(Post::DELETE_IMAGE[URI], 'deleteImage')->name('delete-image');
            Route::get(Post::GET_VARIATIONS[URI], 'getVariations')->name('get-variations');
            Route::post(Post::UPDATE_QUANTITY[URI], 'updateQuantity')->name('update-quantity');
            Route::get(Post::BULK_IMPORT[URI], 'getBulkImportView')->name('bulk-import');
            Route::post(Post::BULK_IMPORT[URI], 'importBulkProduct');
            Route::get(Post::UPDATED_PRODUCT_LIST[URI], 'updatedProductList')->name('updated-product-list');
            Route::post(Post::UPDATED_SHIPPING[URI], 'updatedShipping')->name('updated-shipping');
            Route::post(Post::DENY[URI], 'deny')->name('deny');
            Route::post(Post::APPROVE_STATUS[URI], 'approveStatus')->name('approve-status');
            Route::get(Post::SEARCH[URI], 'getSearchedProductsView')->name('search-product');
            Route::get(Post::MULTIPLE_PRODUCT_DETAILS[URI], 'getMultipleProductDetailsView')->name('multiple-product-details');
            Route::get(Post::PRODUCT_GALLERY[URI], 'getProductGalleryView')->name('product-gallery');
            Route::get(Post::STOCK_LIMIT_STATUS[URI] . '/{type}', 'getStockLimitStatus')->name('stock-limit-status');
            Route::post(Post::DELETE_PREVIEW_FILE[URI], 'deletePreviewFile')->name('delete-preview-file');
        });
    });
    // Banner
