...
@push('script')
    ...
    if (!categoryWrapper || !scrollLeftBtn || !scrollRightBtn) {
        console.error("❌ خطأ: بعض العناصر لم يتم العثور عليها في الصفحة!");
        return;
    }
    ...
    function updateScrollButtons() {
        ...
        console.log("categoryWrapper:  ",categoryWrapper.clientWidth);
        console.log("categoryWrapper:  ",categoryWrapper.clientWidth);
        scrollLeftBtn.style.display = categoryWrapper.scrollLeft > 0 ? "flex" : "none";
        scrollRightBtn.style.display = categoryWrapper.scrollLeft < maxScrollLeft ? "flex" : "none";
    }
    ...
    categoryWrapper.addEventListener("scroll", updateScrollButtons);
    window.addEventListener("resize", updateScrollButtons);

    setTimeout(updateScrollButtons, 500); // تأخير قصير للتأكد من التحديث بعد تحميل الصفحة
});
