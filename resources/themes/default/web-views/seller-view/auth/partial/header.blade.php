    ...
    <div class="">
        <div class="d-flex justify-content-center">
            ...
            <div class="ext-center">
                @if($isoffice)
                <h3 class="mb-2 text-capitalize">{{$vendorRegistrationHeader?->title ?? translate('office_registration')}}</h3>
                @else
                <h3 class="mb-2 text-capitalize">{{$vendorRegistrationHeader?->title ?? translate('vendor_registration')}}</h3>
                @endif
                
                <p>{{$vendorRegistrationHeader?->sub_title ?? translate('already_have_account').'?'}}
                <!-- <p>{{$vendorRegistrationHeader?->sub_title ?? translate('create_your_own_store').'.'.translate('already_have_account').'?'}} -->
