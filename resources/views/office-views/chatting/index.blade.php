...
@section('content')
    <div class="content container-fluid">
                    ...
                    <div class="inbox_people">
                        <form class="search-form mb-4 px-20" id="chat-search-form">
                            <div class="search-input-group">
                                ...
                                @if($userType=="office")
                                <input id="myInput" type="text" aria-label="Search offices..."
                                placeholder="{{ translate('search_offices') }}...">
                                @else
                                <input id="myInput" type="text" aria-label="Search customers..."
                                       placeholder="{{ request('type') == 'admin' ? translate('search_admin') : translate('search_delivery_men')}}...">
                                @endif
                            </div>
                        </form>
                        <ul class="nav nav-tabs gap-3 border-0 mb-3 mx-4" id="pills-tab" role="tablist">
                            ...
                            <li class="nav-item" role="presentation">
                                <a class="nav-link bg-transparent p-2 {{ request('type') == 'factory' ? 'active' : '' }}"
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="customers" role="tabpanel"
                                ...
                                <div class="inbox_chat d-flex flex-column">
                                    ...
                                    @else
                                        @if(isset($factories) && count($factories) > 0)
                                            @foreach($factories as $key=> $factory)
                                                @if($factory->id )
                                                    <div class="list_filter">
                                                        <div
                                                            class="chat_list p-3 d-flex gap-2 @if ($key == 0) bg-soft-secondary @endif get-ajax-message-view"
                                                            data-user-id="{{ $factory->id }}">
                                                            <div class="chat_people media gap-10 w-100" id="chat_people">
                                                                <div class="chat_img avatar avatar-sm avatar-circle">
                                                                    <img src="{{ getStorageImages(path:$factory->image_full_url,type: 'backend-profile') }}"
                                                                        id="{{$factory->id}}"
                                                                        class="avatar-img avatar-circle" alt="">
                                                                    <span
                                                                        class="avatar-status avatar-sm-status avatar-status-success"></span>
                                                                ...
                                                                <div class="chat_ib media-body">
                                                                    <h5 class="mb-1 seller {{$factory->seen_by_seller ?'active-text' :''}}"
                                                                        id="{{ $factory->id }}"
                                                                        data-name="{{ $factory->f_name.' '.$factory->l_name }}"
                                                                        data-phone="{{ $factory->phone }}">
                                                                        {{ $factory->f_name .' '. $factory->l_name }}

                                                                        <span class="lead small float-end">{{ $factory->created_at?->diffForHumans() }}</span>
                                                                    </h5>
                                                                    <span
                                                                        class="mt-2 font-weight-normal text-muted d-block"
                                                                        id="{{ $factory->id }}"
                                                                        data-name="{{ $factory->f_name .' '. $factory->l_name}}"
                                                                        data-phone="{{ $factory->phone }}">{{ $factory->phone }}</span>
                                                                </div>
                                                            </div>
                                                            @if(!$factory->seen_by_admin && !($key == 0))
                                                                <div
                                                                    class="message-status bg-danger notify-alert-{{ $factory->id }}"></div>
                                                            @endif
                                                        </div>
                                                    </div>
                                                @endif
                ...
                <div class="card card-body card-chat justify-content-center Chat" id="">
                    @if(isset($lastChatUser))
                        ...
                        <div class="card-body p-3 overflow-y-auto height-220 flex-grow-1 msg_history d-flex flex-column-reverse" id="chatting-messages-section">
                            @include('office-views.chatting.messages', ['lastChatUser'=>$lastChatUser, 'chattingMessages'=>$chattingMessages])
                        </div>
                        ...
                        <div class="type_msg">
                            <div class="input_msg_write">
                                <form class="mt-4 chatting-messages-ajax-form" enctype="multipart/form-data">
                                    @csrf
                                    @if($userType=="factory")
                                    <input type="hidden" id="current-user-hidden-id" value="{{ $lastChatUser->id }}" name="seller_id">
                                    @elseif($userType=="admin")
                                    <input type="hidden" id="current-user-hidden-id" value="{{ $lastChatUser->id }}" name="admin_id">
                                    @else
                    ...
                    @elseif($userType=="factory" && count($factories)>0)
                        <div class="inbox_msg_header d-flex flex-wrap gap-3 justify-content-between align-items-center border px-3 py-2 rounded mb-4">
                            <div class="media align-items-center gap-3">
                                <div class="avatar avatar-sm avatar-circle border">
                                    <img class="avatar-img user-avatar-image" id="profile_image"
                                         src="{{ getStorageImages(path: $office->image_full_url,type: 'backend-profile') }}"
                                         alt="Image Description">
                                    <span class="avatar-status avatar-sm-status avatar-status-success"></span>
                                </div>
                                <div class="media-body">
                        ...
                        <div class="card-body p-3 overflow-y-auto height-220 flex-grow-1 msg_history d-flex flex-column-reverse" id="chatting-messages-section">
                            @include('admin-views.chatting.messages', ['lastChatUser'=>$factories->first(), 'chattingMessages'=>$chattingMessages])
                        </div>
                        ...
                        <div class="type_msg">
                            <div class="input_msg_write">
                                <form class="mt-4 chatting-messages-ajax-form" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" id="current-user-hidden-id" value="{{ $factories->first()->id }}" name="seller_id">
                                    <div class="position-relative d-flex">
                                        <div class="d-flex align-items-center m-0 position-absolute top-3 px-3 gap-2">
                                            <label class="py-0 cursor-pointer">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
                    ...
                    @elseif($userType=="admin")
                        ...
                        <div class="type_msg">
                            <div class="input_msg_write">
                                <form class="mt-4 chatting-messages-ajax-form" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" id="current-user-hidden-id" value="{{ $factories->first()->id }}" name="admin_id">
                                    <div class="position-relative d-flex">
                                        <div class="d-flex align-items-center m-0 position-absolute top-3 px-3 gap-2">
                                            <label class="py-0 cursor-pointer">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
        ...
        @if(Request::is('office/messages/index/factory') || Request::is('office/messages/index/admin'))
        <span id="chatting-post-url"
        data-url="{{Request::is('office/messages/index/admin')? route('office.messages.message').'?admin_id=' : route('office.messages.message').'?seller_id=' }}"></span>
        @else
        <span id="chatting-post-url"
        data-url="{{ Request::is('office/messages/index/customer') ? route('office.messages.message').'?user_id=' :  route('office.messages.message').'?delivery_man_id=' }}"></span>
        @endif
...
@push('script')
    <script src="{{dynamicAsset(path: 'public/assets/back-end/js/office/chatting.js')}}"></script>
