        ...
        <div class="card">
            <div class="border-bottom px-4 py-3">
                <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2 text-capitalize">
                    ...
                    {{translate('referral-vendor')}}
                </h5>
            </div>
            <div class="card-body">
                <form action="{{route('admin.business-settings.referral-vendor.update')}}" method="post">
                    @csrf
                    <span class="text-capitalize d-flex gap-2">{{translate("Add commission for referral's Seller")}}%</span>
                    @php($commission=getWebConfig('sales_commission_referral'))
                    <input type="number"  value="{{$commission ?? 0}}" name="rate" id="rate" class="form-control">
                    <span class="text-capitalize d-flex gap-2">{{translate("The percentage is from the admin’s commission percentage. For example, if you enter the value 20 and the admin takes 30 percent from the merchant, then the merchant who was given the referral will take 20 percent of the admin’s commission.")}}</span>    
                    <button class="btn btn-primary d-flex gap-2">{{translate("Save")}}</button>
                </form>
            </div>
        </div>
    </div>
