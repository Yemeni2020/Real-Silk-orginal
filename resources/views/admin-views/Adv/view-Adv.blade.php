...
@section('content')
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body text-start">
                        <form action="{{ route('admin.adver.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div>
                                        @foreach($languages as $lang)
                                            <div
                                                ...
                                                id="{{ $lang}}-form">
                                                <label class="title-color">{{ translate('Adv_Name') }}<span
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="_link"  class="form-control" id="_link">
                                    </div>
                                    <div class="form-group">
                ...
                <div class="card">
                        ...
                        <div class="d-flex flex-wrap justify-content-between gap-3 align-items-center">
                            <div class="">
                                <h5 class="text-capitalize d-flex gap-1">
                                    ...
                                    <span
                                        class="badge badge-soft-dark radius-50 fz-12">{{ $Adv->total() }}</span>
                                </h5>
                            </div>
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="" type="search" name="searchValue" class="form-control"
                                               placeholder="{{ translate('search_by_Adv_name') }}"
                    ...
                    <div class="table-responsive">
                        <table
                            ...
                            <tr>
                                <th>{{ translate('ID') }}</th>
                                <th class="text-center">{{ translate('Adv_Image') }}</th>
                            ...
                            @foreach($Adv as $key=>$adv)
                                <tr>
                                    ...
                                    <td class="d-flex justify-content-center">
                                        <div class="avatar-60 d-flex align-items-center rounded">
                                            <img class="img-fluid" alt=""
                                                 src="{{ getStorageImages(path: $adv->icon_full_url, type: 'backend-category') }}">
                                        </div>
                                    </td>
                                    <td>{{ $adv['title'] }}</td>
                                    ...
                                    <td class="text-center">
                                        {{ $adv->AdvCategory?->name }}
                                    </td>
                                    <td class="text-center">

                                        <form action="{{ route('admin.adver.status') }}" method="post"
                                              id="category-status{{ $adv['id'] }}-form">
                                            @csrf
                                            <input type="hidden" name="id" value="{{ $adv['id'] }}">
                                            <label class="switcher mx-auto">
                                                <input type="checkbox" class="switcher_input toggle-switch-message"
                                                       name="home_status"
                                                       id="category-status{{ $adv['id'] }}" value="1"
                                                       {{ $adv['status'] == 1 ? 'checked' : '' }}
                                                       data-modal-id="toggle-status-modal"
                                                       data-toggle-id="category-status{{ $adv['id'] }}"
                                                       data-on-image="category-status-on.png"
                                                       data-off-image="category-status-off.png"
                                                       data-on-title="{{ translate('Want_to_Turn_ON').' '.$adv['defaultname'].' '. translate('status') }}"
                                                       data-off-title="{{ translate('Want_to_Turn_OFF').' '.$adv['defaultname'].' '.translate('status') }}"
                                                       data-on-message="<p>{{ translate('if_enabled_this_advertisement_it_will_be_visible_from_the_advertisement_wise_product_section_in_the_website_and_customer_app_in_the_homepage') }}</p>"
                                                       data-off-message="<p>{{ translate('if_disabled_this_advertisement_it_will_be_hidden_from_the_advertisement_wise_product_section_in_the_website_and_customer_app_in_the_homepage') }}</p>">
                                                <span class="switcher_control"></span>
                                            </label>
                                        </form>
                                    </td>
                                    <td>
                                        <div class="d-flex justify-content-center gap-10">
                                            <a class="btn btn-outline-info btn-sm square-btn "
                                               ...
                                               href="{{ route('admin.adver.update',[$adv['id']]) }}">
                                                <i class="tio-edit"></i>
                                            </a>
    ...
    <span id="get-categories" data-categories="{{ json_encode($Adv) }}"></span>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-lg">
                ...
                <div class="modal-body px-4 px-sm-5 pt-0 pb-sm-5">
                    ...
                    <form action="{{ route('admin.adver.delete') }}" method="post" class="product-category-update-form-submit">
                        @csrf
                        <input name="id" hidden="">
                        <div class="gap-2 mb-3">
                            <label class="title-color"
                                   for="exampleFormControlSelect1">{{ translate('select_advertisement') }}
