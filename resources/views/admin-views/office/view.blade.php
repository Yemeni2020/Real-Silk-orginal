...
@section('title', $seller?->shop->name ?? translate("office_Name"))
                ...
                <div class="d-flex flex-wrap gap-3 justify-content-between">
                    <div class="media flex-column flex-sm-row gap-3">
                        ...
                        <div class="media-body">
                            ...
                            @endif
                            <div class="d-block">
                                <h2 class="mb-2 pb-1">{{ $seller->shop? $seller->shop->name : translate("office_Name")." : ".translate("update_Please") }}</h2>
                                <div class="d-flex gap-3 flex-wrap mb-3 lh-1">
                                    ...
                                    <span class="border-left"></span>
                                    <a href="{{ $seller['status']!="pending" ? route('admin.offices.view',['id'=>$seller['id'], 'tab'=>'review']): 'javascript:' }}"
                    ...
                    @if ($seller['status']=="pending")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.offices.updateStatus')}}" id="reject-form" method="POST">
                                ...
                                <button type="button" class="btn btn-danger px-5 form-alert" data-message="{{translate('want_to_reject_this_office').'?'}}" data-id="reject-form">{{translate('reject')}}</button>
                            </form>
                            <form class="d-inline-block" action="{{route('admin.offices.updateStatus')}}" id="approve-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                                <input type="hidden" name="status" value="approved">
                                <button type="button" class="btn btn-success px-5 form-alert" data-message="{{translate('want_to_approve_this_office').'?'}}" data-id="approve-form">{{translate('approve')}}</button>
                            </form>
                        </div>
                    @endif
                    @if ($seller['status']=="approved")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.offices.updateStatus')}}" id="suspend-form" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                                <input type="hidden" name="status" value="suspended">
                                <button type="button" class="btn btn-danger px-5 form-alert" data-message="{{translate('want_to_suspend_this_office').'?'}}" data-id="suspend-form">{{translate('suspend_this_office')}}</button>
                            </form>
                        </div>
                    @endif
                    @if ($seller['status']=="suspended" || $seller['status']=="rejected")
                        <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                            <form class="d-inline-block" action="{{route('admin.offices.updateStatus')}}" id="active-form" method="POST">
                                ...
                                <input type="hidden" name="id" value="{{$seller['id']}}">
                                <input type="hidden" name="status" value="approved">
                                <button type="button" class="btn btn-success px-5 form-alert" data-message="{{translate('want_to_active_this_office').'?'}}" data-id="active-form">{{translate('active')}}</button>
                            </form>
                        </div>
                    @endif
                </div>
                ...
                <div class="d-flex gap-3 flex-wrap flex-lg-nowrap">
                    ...
                    <div class="row gy-3 flex-grow-1 w-100">
                        <div class="col-sm-6 col-xxl-3">
                            <h4 class="mb-3 text-capitalize">{{translate('office_information')}}</h4>

                            <div class="pair-list">
                                <div>
                                    <span class="key text-nowrap">{{translate('office_name')}}</span>
                                    <span>:</span>
